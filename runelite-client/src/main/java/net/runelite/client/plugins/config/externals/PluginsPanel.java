...
public class PluginsPanel extends JPanel
{
	...
	private final ContainableFrame frame = new ContainableFrame();
	...
	@Subscribe
	public void onOPRSRepositoryChanged(OPRSRepositoryChanged event)
	{
		buildFilter();
		reloadPlugins();
		repaint();
	...
	private void fetchPlugins()
	{
		...
		{
			JOptionPane.showMessageDialog(frame, "The external plugin list could not be loaded.", "Error", JOptionPane.ERROR_MESSAGE);
			return;
		}
	...
	@Subscribe
	private void onOPRSPluginChanged(OPRSPluginChanged externalPluginChanged)
	{
		...
		install.addMouseListener(new MouseAdapter()
		{
			...
			public void mousePressed(MouseEvent e)
			{
				if (installed)
				{
					if (hideAction)
					{
						JOptionPane.showMessageDialog(frame, "This plugin can't be uninstalled because one or more other plugins have a dependency on it.", "Error!", JOptionPane.ERROR_MESSAGE);
					}
					else
					{
	...
	private boolean installPlugin(PluginInfo pluginInfo)
	{
		...
		catch (VerifyException ex)
		{
			...
			{
				SwingUtil.syncExec(() ->
					JOptionPane.showMessageDialog(frame, pluginInfo.name + " could not be installed, the hash could not be verified.", "Error!", JOptionPane.ERROR_MESSAGE));
			}
