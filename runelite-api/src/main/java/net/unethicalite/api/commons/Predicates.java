...
public class Predicates
{
	...
	public static <T extends Identifiable> Predicate<T> ids(int... ids)
	{
		return t ->
		{
			for (int id : ids)
			{
				if (t.getId() == id || (t instanceof TileObject && ((TileObject) t).getId() == id))
				{
					return true;
				}
			}
	...
	{
		return t -> t instanceof TileObject ? ids.contains(((TileObject) t).getId()) || ids.contains(t.getId()) : ids.contains(t.getId());
	}
}
